...
const AGENT_ORDER = ['planner', 'researcher', 'quant', 'qual', 'moderator', 'writer', 'risk']
...
export default function Dashboard() {
  ...
  async function handleResearch(e: React.FormEvent, overrideQuery?: string) {
    e.preventDefault()
          ...
          {currentReport && !loading && (
            <div className="max-w-4xl">
              <div className="mb-6 pb-6 border-b border-[#1a1b28]">
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="flex items-center gap-2">
                    {/* Re-run button */}
                    <button
                      onClick={(e) => handleResearch(e, currentReport.query)}
                      className="flex items-center gap-2 text-gray-400 hover:text-white font-mono text-xs border border-[#1a1b28] hover:border-[#2a2b38] px-3 py-2 rounded-lg transition-all hover:bg-[#0f1018]"
                      title="Re-run this query with fresh data"
                    >
              ...
              {currentReport.risk_flags && (
                <div className="mt-8 rounded-lg border border-red-500/20 bg-red-500/5 p-6">
                  <div className="flex items-center gap-2 mb-4">
                    <span className="text-base">🚨</span>
                    <p className="text-red-400 font-mono text-xs font-semibold uppercase tracking-widest">
                      Risk Flags
                    </p>
                  </div>
                  <ReactMarkdown
                    remarkPlugins={[remarkGfm]}
                    components={{
                      p: ({children}) => <p className="text-gray-300 font-mono text-sm leading-relaxed mb-3">{children}</p>,
                      strong: ({children}) => <strong className="text-red-300 font-semibold">{children}</strong>,
                      ul: ({children}) => <ul className="space-y-2 mb-3">{children}</ul>,
                      li: ({children}) => <li className="text-gray-300 font-mono text-sm flex gap-2"><span className="text-red-400 mt-0.5 flex-shrink-0">·</span><span>{children}</span></li>,
                    }}
                  >
                    {currentReport.risk_flags}
                  </ReactMarkdown>
